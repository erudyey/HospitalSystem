...
class ClinicApp:
    ...
    def show_database_error(error):
        messagebox.showerror("Database Error", f"The change could not be saved.\n\n{error}")

    def setup_registration_tab(self):
        self.registration_tab.columnconfigure(1, weight=1)
        ttk.Label(self.registration_tab, text="Patient Registration", font=("Arial", 18, "bold")).grid(
            row=0, column=0, columnspan=2, padx=20, pady=(25, 30), sticky="w"
        )
        self.name_entry = self.add_labeled_entry(self.registration_tab, "Full Name:", 1)
        self.contact_entry = self.add_labeled_entry(self.registration_tab, "Contact:", 2)
        ...
        ttk.Button(buttons, text="Register Patient", command=self.save_patient).grid(row=0, column=0, padx=10)
        ttk.Button(buttons, text="Clear", command=self.clear_patient_form).grid(row=0, column=1, padx=10)

    @staticmethod
    def add_labeled_entry(parent, label, row):
        ttk.Label(parent, text=label).grid(row=row, column=0, padx=20, pady=12, sticky="w")
    ...
    def save_patient(self):
        ...
        try:
            ...
            messagebox.showwarning("Invalid Age", "Enter a positive whole number for age.")
            return
        try:
    ...
    def setup_booking_tab(self):
        ...
        ttk.Label(self.booking_tab, text="Appointment Booking", font=("Arial", 18, "bold")).grid(
            row=0, column=0, columnspan=3, padx=20, pady=(25, 30), sticky="w"
        )
        ttk.Label(self.booking_tab, text="Select Patient:").grid(row=1, column=0, padx=15, pady=12, sticky="w")
        self.booking_patient_combo = ttk.Combobox(self.booking_tab, state="readonly", width=35)
        self.booking_patient_combo.grid(row=1, column=1, padx=15, pady=12, sticky="w")
        ttk.Button(self.booking_tab, text="Refresh Patients", command=self.refresh_patient_views).grid(
            row=1, column=2, padx=10, pady=12
        )
        self.doctor_entry = self.add_labeled_entry(self.booking_tab, "Doctor Name:", 2)
        self.date_entry = self.add_labeled_entry(self.booking_tab, "Date (YYYY-MM-DD):", 3)
        buttons = ttk.Frame(self.booking_tab)
        buttons.grid(row=4, column=0, columnspan=3, pady=30)
        ttk.Button(buttons, text="Book Appointment", command=self.save_booking).grid(row=0, column=0, padx=10)
        ttk.Button(buttons, text="Clear", command=self.clear_booking_form).grid(row=0, column=1, padx=10)
    ...
    def setup_status_tab(self):
        ...
        ttk.Label(self.status_tab, text="Appointment Status", font=("Arial", 18, "bold")).grid(
            row=0, column=0, columnspan=4, padx=20, pady=(25, 20), sticky="w"
        )
        ttk.Label(self.status_tab, text="Select Patient:").grid(row=1, column=0, padx=10, pady=10, sticky="e")
        self.status_patient_combo = ttk.Combobox(self.status_tab, state="readonly", width=25)
        self.status_patient_combo.grid(row=1, column=1, padx=10, pady=10, sticky="w")
        ttk.Button(self.status_tab, text="Refresh Patients", command=self.refresh_patient_views).grid(
            row=1, column=2, padx=5, pady=10
        )
        ttk.Button(self.status_tab, text="View Appointments", command=self.view_patient_appointments).grid(
        ...
        for column, width, anchor in zip(columns, (110, 180, 120, 170, 120, 120), ("center", "w", "center", "w", "center", "center")):
        ...
        ttk.Button(buttons, text="Mark Completed", command=lambda: self.set_selected_appointment_status("Completed")).grid(row=0, column=0, padx=8)
        ttk.Button(buttons, text="Cancel Appointment", command=lambda: self.set_selected_appointment_status("Cancelled")).grid(row=0, column=1, padx=8)
        ttk.Button(buttons, text="Refresh", command=self.view_patient_appointments).grid(row=0, column=2, padx=8)
    ...
    def set_selected_appointment_status(self, status):
        ...
        if not selection:
            messagebox.showwarning("Missing Selection", "Select an appointment from the table first.")
            return
        appointment_id = self.status_tree.item(selection[0], "values")[0]
        if not messagebox.askyesno("Confirm Update", f"Mark appointment #{appointment_id} as {status}?"):
            return
        try:
            if not db.update_appointment_status(appointment_id, status):
                messagebox.showwarning("Appointment Not Found", "The selected appointment no longer exists.")
                return
        except sqlite3.Error as error:
            self.show_database_error(error)
            return
        messagebox.showinfo("Updated", f"Appointment #{appointment_id} marked as {status}.")
    ...
    def setup_records_tab(self):
        ...
        ttk.Label(self.records_tab, text="Patient Records", font=("Arial", 18, "bold")).grid(
            row=0, column=0, padx=20, pady=(25, 20), sticky="w"
        )
        ...
        for column, width, anchor in zip(columns, (80, 250, 180, 100), ("center", "w", "center", "center")):
        ...
        ttk.Button(self.records_tab, text="Refresh Data", command=self.refresh_patient_views).grid(row=2, column=0, pady=15)
